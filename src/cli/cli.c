...
static const char cli_skill_content[] =
    "---\n"
    "name: codebase-memory\n"
    "description: Codebase knowledge graph expert. ALWAYS invoke this skill before code reviews, "
    "before planning changes, when exploring unfamiliar code, when tracing impact, when checking "
    "architecture, routes, dependencies, dead code, complexity, or refactor risk. Use this before "
    "Grep, Glob, or file search for code discovery. CLI mode is the only mechanism for this skill; "
    "do not use MCP tools unless the user explicitly asks.\n"
    "---\n"
    "\n"
    "# Codebase Memory — CLI Skill\n"
    "\n"
    "Use `codebase-memory-mcp` as a normal executable. Do not rely on a permanent MCP connection. "
    "Run each request through bash:\n"
    "\n"
    "```bash\n"
    ...
    "Use this skill before grep/file search when the task is about code understanding. In "
    "particular:\n"
    "- Before edits: map relevant functions, routes, files, dependencies, and likely blast "
    "radius.\n"
    "- Before code review: inspect changed symbols, callers, callees, hotspots, coverage gaps, and "
    ...
    "- For quality cleanup: find dead code candidates, high-degree functions, complex areas, and "
    "duplicates.\n"
    "\n"
    "## First checks\n"
    "```bash\n"
    ...
    "```\n"
    "\n"
    "If binary is missing, ask the user before installing. Do not auto-install. If the user says "
    "yes, install binary-only, not MCP config:\n"
    "```bash\n"
    ...
    "4. `get_graph_schema` — learn available labels, edge types, and properties before custom "
    "queries.\n"
    "5. `search_graph` — find relevant functions, classes, routes, resources, or files before "
    "grep.\n"
    ...
    "8. `check_index_coverage` — check candidate source paths before making negative, exhaustive, "
    "or risky claims.\n"
    ...
    "```bash\n"
    "project=PROJECT\n"
    "json=$(jq -nc --arg project \"$project\" --arg q \"auth handler\" "
    "'{project:$project,query:$q,limit:20}')\n"
    "codebase-memory-mcp cli search_graph \"$json\"\n"
    "```\n"
    "\n"
    "## Evidence tiers\n"
    "- **Scout (Tier 1):** fast positive lookup with few graph calls and targeted source checks. "
    "Treat results as provisional. Do not make absence, exhaustive, dead-code, or complete-impact "
    "claims.\n"
    "- **Verify (Tier 2, default):** task-directed searches, relevant trace directions, exact "
    "snippets for material claims, all relevant result pages, and coverage checks for cited "
    "paths.\n"
    "- **Auditor (Tier 3):** bounded-scope full verification with current graph generation, "
    "complete relevant pagination, both call directions and broader relationships when material, "
    "plus explicit unresolved limitations.\n"
    "- **Every tier:** after candidate paths are known, call `check_index_coverage` once with "
    "every evidence path. For negative or exhaustive claims also include relevant scopes. A clean "
    "result means no recorded gap, not proof of completeness. For partial, skipped, excluded, "
    "stale, pending, or unknown coverage, read/grep the reported ranges or scope before relying on "
    "the graph.\n"
    "\n"
    ...
    "exploration.\n"
    "- Before delegating, query the graph and coverage in the parent. Pass tier, exact project, "
    "generation/freshness, bounded scope, queries and pagination state, qualified symbols, paths, "
    "call-chain findings, coverage ranges/reasons, source fallback already performed, and "
    "unresolved questions to the child.\n"
    "- A child without CLI access must not claim graph access. It should work from supplied "
    "evidence and use read/grep on exact source, especially every reported missed-coverage range.\n"
    "\n"
    ...
    "- `search_graph` — primary discovery command for symbols, routes, files, resources, and "
    "quality filters.\n"
    "- `search_code` — indexed text search. Use for literals, error strings, config keys, or "
    "comments.\n"
    ...
    "- `detect_changes` — review helper for current git diff, changed symbols, blast radius, and "
    "risk.\n"
    ...
    "- `query_graph` — custom Cypher-like read queries for advanced architecture/quality "
    "questions.\n"
    "- `check_index_coverage` — identify partial, skipped, excluded, stale, pending, or unknown "
    "indexed source ranges.\n"
    "- `manage_adr` — read or update Architecture Decision Records. Ask before creating/updating "
    "ADRs.\n"
    ...
    "'{\"repo_path\":\"/path/to/repo\",\"name\":\"my-project\",\"mode\":\"moderate\"}'\n"
    "\n"
    "# Find symbols or concepts before editing\n"
    "codebase-memory-mcp cli search_graph '{\"project\":\"PROJECT\",\"query\":\"update "
    "settings\",\"limit\":20}'\n"
    "\n"
    "# Find HTTP routes before changing API behavior\n"
    "codebase-memory-mcp cli search_graph "
    "'{\"project\":\"PROJECT\",\"label\":\"Route\",\"limit\":100}'\n"
    "\n"
    "# Read implementation after search_graph returns qualified_name\n"
    "codebase-memory-mcp cli get_code_snippet "
    "'{\"project\":\"PROJECT\",\"qualified_name\":\"pkg/"
    "orders.ProcessOrder\",\"include_neighbors\":true}'\n"
    "\n"
    "# Trace impact before edits or during review\n"
    "codebase-memory-mcp cli trace_path "
    "'{\"project\":\"PROJECT\",\"function_name\":\"ProcessOrder\",\"direction\":\"both\",\"depth\":"
    "3}'\n"
    "\n"
    "# Check coverage for cited source paths\n"
    "codebase-memory-mcp cli check_index_coverage "
    "'{\"project\":\"PROJECT\",\"paths\":[\"src/orders.py\"]}'\n"
    "\n"
    "# Review current git diff with graph context\n"
    "codebase-memory-mcp cli detect_changes "
    "'{\"project\":\"PROJECT\",\"repo_path\":\"/path/to/repo\"}'\n"
    "\n"
    "# Architecture overview\n"
    "codebase-memory-mcp cli get_architecture "
    "'{\"project\":\"PROJECT\",\"aspects\":[\"packages\",\"dependencies\",\"clusters\",\"routes\"]}"
    "'\n"
    "\n"
    "# Advanced quality query\n"
    "codebase-memory-mcp cli query_graph '{\"project\":\"PROJECT\",\"query\":\"MATCH (f:Function) "
    "RETURN f.qualified_name LIMIT 20\",\"max_rows\":20}'\n"
    "```\n"
    "\n"
    "## Edge types\n"
    "CALLS, HTTP_CALLS, ASYNC_CALLS, DATA_FLOWS, IMPORTS, DEFINES, DEFINES_METHOD, HANDLES, "
    "IMPLEMENTS, OVERRIDE, USAGE, CONFIGURES, FILE_CHANGES_WITH, SIMILAR_TO, SEMANTICALLY_RELATED, "
    "CONTAINS_FILE, CONTAINS_FOLDER, CONTAINS_PACKAGE\n"
    "\n"
    "## Gotchas\n"
    "1. `search_graph(relationship=\"HTTP_CALLS\")` filters nodes by degree. Use `query_graph` "
    "with Cypher to see actual edges.\n"
    ...
    "- Use this before grep/file search for code discovery, review pre-analysis, and change "
    "planning.\n"
    "- Use `jq` for JSON creation and summarizing large outputs.\n"
    "- Save large raw outputs to `/tmp/cbm-*.json`.\n"
    "- Fall back to file search only for non-indexed files, exact literals missing from graph, "
    "coverage gaps, or generated/vendor files.\n"
...
int cbm_install_agent_configs(const char *home, const char *binary_path, bool force, bool dry_run) {
    ...
    if (g_cli_skill_mode) {
        if (!agents.claude_code && !g_install_plan) {
            printf(
                "skill-mode: cli requested, but no Claude Code skills directory was detected.\n");
        }
        return CLI_OK;
    }
